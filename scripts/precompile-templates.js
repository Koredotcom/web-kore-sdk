 ...
 * variable declarations for data access, making them bundleable by webpack.
 *
 * Templates are split into separate files per module:
 *   - src/libs/precompiled-templates-widgets.js (widget templates)
 *   - src/libs/precompiled-templates-v2chatwindow.js (V2 chat window templates)
 *   - src/plugins/v2Plugin/precompiled-templates-v2plugin.js (V2 plugin templates)
 *   - src/plugins/v2WidgetsPlugin/precompiled-templates-v2widgets.js (V2 widget templates)
 *   - src/plugins/searchSuggestions/precompiled-templates-search.js (search suggestions)
 *   - src/plugins/solutions/precompiled-templates-solutions.js (solutions templates)
 *
...
function main() {
  ...
  // Split templates into groups based on which bundle they belong to
  const groups = {
    'widgets': { ids: [], path: 'libs/precompiled-templates-widgets.js' },
    'v2ChatWindow': { ids: [], path: 'libs/precompiled-templates-v2chatwindow.js' },
    'v2Plugin': { ids: [], path: 'plugins/v2Plugin/precompiled-templates-v2plugin.js' },
    'v2Widgets': { ids: [], path: 'plugins/v2WidgetsPlugin/precompiled-templates-v2widgets.js' },
    'searchSuggestions': { ids: [], path: 'plugins/searchSuggestions/precompiled-templates-search.js' },
    'solutions': { ids: [], path: 'plugins/solutions/precompiled-templates-solutions.js' },
  };

  for (const [id] of allTemplates) {
    if (id === 'chat_window_tmpl' || id === 'kore_popup_tmpl') {
      groups.v2ChatWindow.ids.push(id);
    } else if (id.startsWith('v2_widget_')) {
      groups.v2Widgets.ids.push(id);
    } else if (id.startsWith('v2_chat_')) {
      groups.v2Plugin.ids.push(id);
    } else if (id === 'search_message_tmpl' || id === 'quick_replies_tmpl') {
      groups.solutions.ids.push(id);
    } else if (id === 'chat_autoSuggestion_tmpl') {
      groups.searchSuggestions.ids.push(id);
    } else {
      groups.widgets.ids.push(id);
    }
  }
  ...
  for (const [groupName, group] of Object.entries(groups)) {
    ...
    const outputFile = path.resolve(SRC_DIR, group.path);
    ...
    for (let i = 0; i < group.ids.length; i++) {
      ...
      const data = allTemplates.get(id);
      const comma = i < group.ids.length - 1 ? ',' : '';
    ...
    const relPath = path.relative(path.resolve(__dirname, '..'), outputFile);
    const sizeKB = (output.length / 1024).toFixed(1);
    console.log(`  -> ${relPath} (${group.ids.length} templates, ${sizeKB}KB)`);
  }

  console.log(`\nCompiled ${allTemplates.size} templates into ${Object.values(groups).filter(g => g.ids.length > 0).length} files.`);
  console.log('Done.');
}
