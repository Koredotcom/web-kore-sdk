(function($){
	...
	customTemplate.prototype.renderMessage = function (msgData) {
		...
		} else if (msgData.message[0] && msgData.message[0].component && msgData.message[0].component.payload && msgData.message[0].component.payload.template_type == "live_person") {
			// this is a temporary fix
			if (typeof msgData.message[0].cInfo.body === 'object') {
				msgData.message[0].cInfo.body = msgData.message[0].component.payload.text;
			}

			messageHtml = $(this.getChatTemplate("livePersonTemplate")).tmpl({
				'msgData': msgData,
				'helpers': this.helpers,
				'extension': this.extension
			});
...
print(JSON.stringify(message)); */
	...
	';
		...
		} else if (tempType === "removingButtonTemplate") {
			return removingButtonTemplate;
		} else if (tempType === "livePersonTemplate") {
			return msgTemplate;
		} else {
			return "";
		}
