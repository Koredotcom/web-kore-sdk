                                    ...
                                    {ele.subInformation && ele.subInformation.length > 0 && <div className="status-data-block">
                                        {ele.subInformation.map((item: any) => (
                                            <div className="status-data">
                                                {item.title && <h2 dangerouslySetInnerHTML={{ __html: helpers.convertMDtoHTML(item.title, "bot") }}></h2>}
                                                {item.value && <p dangerouslySetInnerHTML={{ __html: helpers.convertMDtoHTML(item.value, "bot") }}></p>}
                                            </div>))}
                                    </div>}
                                </div>
                                ...
                                {ele.subElements.map((se: any) => (
                                    <div className="details-card-status" onClick={(event) => handleEvent(event, se.default_action)}>
                                        <div className="details-card-status-header">
                                            {se.title && <h2 dangerouslySetInnerHTML={{ __html: helpers.convertMDtoHTML(se.title, "bot") }}></h2>}
                                        ...
                                        {se.values && se.values.length && se.values.map((v: any) => (<div className="status-heading">
                                            <h3 dangerouslySetInnerHTML={{ __html: helpers.convertMDtoHTML(v.title, "bot") }}></h3>
                                            <div className="status-info">
                                                {v.icon && <figure>
                                                    <img alt="" src={v.icon} />
                                                </figure>}
                                                <span dangerouslySetInnerHTML={{ __html: helpers.convertMDtoHTML(v.value, "bot") }}></span>
                                            </div>
                                        </div>))}
                                    </div>))}
                            </div>}
